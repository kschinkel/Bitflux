...
var rowSelModel = new Ext.grid.RowSelectionModel({
    ...
    listeners: {
        rowselect: function(smObj, rowIndex, record) {
			...
			}else if(record.data.status == 'queued' || record.data.status == 'error'){
				Ext.getCmp('startBtn').enable();
				Ext.getCmp('stopBtn').enable();
			}else{
				Ext.getCmp('startBtn').disable();
...
Ext.onReady(function(){
    ...
    store = new Ext.data.JsonStore({
        ...
        totalProperty: 'total',
        fields: [
            'filename', 'total_size','queue_id','status','dl_speed','progress','eta','pid', 'nid', 'path','connections'
        ],
            ...
            items: [
                {text: 'Add',
                    handler: function() {
                        ...
                        autoDLForm.getForm().setValues({autoDL_download_to:CWD});
                        autoDLWindow.show();
                    }
                },
                {text: 'Remove',
                ...
                {text: 'Edit',
                    handler: function(){
                        if( autoDLBrowser.selModel.hasSelection() ){
                            var selected = autoDLBrowser.selModel.getSelected();
                            /*var postData = selected.data.id.toString();
                            'entryName',
                            'id',
                            'latest_season',
                            'latest_episode'*/
                            autoDLWindow.show();
                            console.log(CWD);
                            autoDLForm.getForm().setValues({
                                    autoDL_proper_name:selected.data.proper_name,
                                    autoDL_match_names:selected.data.filenames_to_match,
                                    autoDL_season_start:selected.data.latest_season,
                                    autoDL_episode_start:selected.data.latest_episode,
                                    autoDL_download_to:CWD,
                                    autoDL_id:selected.data.id.toString()
                                });
                        }
                    }
                },
    ...
    var autoDLForm = new Ext.form.FormPanel({
        ...
        items: [
                ...
                {fieldLabel: 'Proper Name',
                name: 'autoDL_proper_name',
                allowBlank: false,
                labelSeparator: ':' // override labelSeparator layout config
                },
                {fieldLabel: 'Filenames to Match; comma separated',
                name: 'autoDL_match_names',
                allowBlank: false,
                labelSeparator: ':' // override labelSeparator layout config
                },
                {xtype:'numberfield',
                fieldLabel: 'Season to start from',
                ...
                {xtype:'numberfield',
                fieldLabel: 'Episode to start from',
                ...
                {xtype:'textarea',
                fieldLabel: 'Download to (Season directory will be created)',
                name: 'autoDL_download_to',
                disabled: true,
                labelSeparator: ':' // override labelSeparator layout config
                }
        ],
...
initialize = function() {
    ...
    autoDLStore.setDefaultSort('proper_name','ASC');
